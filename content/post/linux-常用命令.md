...
tags: ["linux", "bash"]
author: ["lavaicer", "qianngchn"]
# author: ["Me", "You"] # multiple authors
---

部分内容转自 [Linux 常用命令行小技巧](https://qianngchn.github.io/wiki/7.html)
...
- 要了解内存状态，可以运行 `free` 和 `vmstat`，看懂它们的输出结果吧。特别是，要知道`cached`值是`Linux`内核为文件缓存所占有的内存，因此，要有效地统计`free`值。
